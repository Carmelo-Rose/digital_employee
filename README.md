...
上传订单 Excel/CSV → 自动识别字段 → 检测 5 类异常 → 生成运营日报 → 网页展示 / 命令行 → 一键推送到企微/飞书。

**项目作用**：把电商运营每天「翻订单表、找异常、写日报」的重复劳动自动化。运营只需上传订单导出文件，系统自动识别异常、统计指标，并生成一份可直接发群的运营日报。
...
- **第四阶段（本次新增）**：可交互、可演示的 **Web Demo**——上传 → 看「数字员工执行过程」→ 预览 AI 日报 → **人工确认**后推送企业微信。
...
- **第 4.5 阶段（修正打磨，本次新增）**：提升严谨性与可演示效果，更适合面试录屏。
...
### mock LLM vs 真实 LLM
- **mock**（默认/离线）：`ReportGenerationNode` 用确定性模板渲染 7 板块，不联网、可复现，适合演示。
...
- **快速落地数字员工**：Web Demo 可上传即跑，节点清晰可讲、可扩展。
...
每份报告末尾的「数据质量提示」让看报告的人**清楚边界**——哪些字段被用上、哪些没被识别、哪些检测因缺字段没做、关键字段是否缺失。避免「没分析」被误读成「没问题」，是 Demo 严谨性的体现。
...
2. 勾选 **使用 AI 日报**（可选 **强制 mock** 离线演示）→ 点 **开始分析**。
3. 看「数字员工执行过程」分步跑完，下方**预览 AI 运营日报**。
4. 点 **✅ 确认并推送到企业微信**。

**配置企业微信 Webhook**（真推时）：在 `.env` 里设置 `WECOM_WEBHOOK_URL=`（企业微信群机器人「添加机器人」里复制的地址）。不配也能演示——会走模拟推送并在页面提示。
...
- **融合 Agent（关键）**：`analyzer.py / report.py / push.py` 均为纯函数模块，
  ...
  「读文件→分析→生成→推送」，本 Demo 的 Web 作为触发入口，共享同一套数据层。
